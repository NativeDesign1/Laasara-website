...
const Navbar = () => {
  ...
  return (
    <nav >
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden absolute top-0 left-0 w-full bg-white transition-all duration-300 ease-in-out ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        <div className="flex flex-col w-full space-y-8 mt-4">
          {menuLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
